...
npm run lint           # eslint
npx tsc --noEmit -p tsconfig.app.json   # typecheck — ИМЕННО app.json: tsconfig.json это solution-файл с "files": [], без --build он не проверяет НИ ОДНОГО файла (CI-шаг «Типы» на нём — холостой; на app.json ~133 старых ошибки в webrtc/hr, смотрите только свои файлы через grep)
npm test               # vitest run
