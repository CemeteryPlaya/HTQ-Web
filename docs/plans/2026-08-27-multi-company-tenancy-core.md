...
⚠️ **Код ниже писался по документации и на живой системе не заработал.**
Реализация разошлась с ним существенно — читайте закоммиченный
`migration_service.py`, а не этот блок. Что именно не сработало и почему:

| Что в эталоне | Что на самом деле |
|---|---|
| `search_path` без `public` весь прогон | Невозможно: `hr.0019`/`tasks.0003` зависят от `django_celery_beat` в `public`. Нужны две фазы — см. докстринг ниже |
| `call_command("migrate", label)` | Безусловно шлёт `post_migrate`, тот лезет в `django_content_type`. Нужен прямой `MigrationExecutor` |
| Ничего про нетенантные аппки | Их миграции обязаны помечаться применёнными без выполнения, иначе `hr.0019` падает на `apps.get_model` |
| Схема может не существовать | Нужен `SchemaMissing`: Postgres уложил бы тенантные таблицы в `public` ПОВЕРХ боевых |
| `--plan` создавал `django_migrations` | Сухой прогон обязан не писать ничего |
| `--to zero` для отката | Обратное направление запрещено: реверс удаляет `PeriodicTask` из `public` у всей группы |

Докстринг ниже приведён в соответствие с реальностью; остальной код блока —
нет, он оставлен как след замысла.
...
migrate: у каждой компании должна быть СВОЯ история миграций. Если состояние
уедет в public.django_migrations, все компании начнут считать себя
мигрированными вместе — при полностью пустых схемах. Ошибка молчаливая и
проявляется только на первом запросе к несуществующей таблице, поэтому она
закреплена тестом test_each_schema_gets_its_own_migration_state.

Механизм двухфазный, и первая фаза неочевидна. Держать search_path без public
весь прогон НЕЛЬЗЯ: hr.0019 и tasks.0003 зависят от django_celery_beat и
пишут PeriodicTask в public. Поэтому:

  1. search_path ставится ТОЛЬКО в схему компании (include_public=False), и в
     этот момент вызывается MigrationRecorder.ensure_schema(). Django
     проверяет наличие таблицы через pg_table_is_visible, то есть по
     search_path; public.django_migrations невидима, и неквалифицированный
     CREATE TABLE кладёт свою django_migrations в схему компании.
  2. Дальше public возвращается в путь ВТОРЫМ. co_<slug>.django_migrations
     затеняет общую по порядку разрешения имён: состояние читается и пишется
     в схему компании, а django_celery_beat разрешается из public.

Пропустить фазу 1 — значит молча писать состояние в public.

Отсюда же три обязательных условия: схема обязана существовать до прогона
(SchemaMissing — иначе Postgres уложит тенантные таблицы в public ПОВЕРХ
боевых); call_command("migrate") использовать нельзя (команда безусловно шлёт
post_migrate, а тот лезет в django_content_type/auth_permission) — нужен
прямой MigrationExecutor; миграции нетенантных аппок помечаются применёнными
без выполнения, потому что _migrate_all_forwards собирает состояние проекта
только из применённых миграций (ветки else: mutate_state там нет, в отличие
от _migrate_all_backwards).

Обратное направление ЗАПРЕЩЕНО: реверс тенантных data-миграций выполняется по
public, и откат ОДНОЙ компании удалил бы платформенные строки PeriodicTask,
выключив периодику всей группы.

Data-миграции с общим эффектом (hr.0019, tasks.0003) исключаются из прогона
по компаниям: в схеме компании они не создают ничего, их эффект глобален и
достигается обычным manage.py migrate, а повторное выполнение вредно —
defaults несут enabled=True и crontab, то есть заведение новой компании молча
включает выключенную оператором задачу.
...
⚠️ В `backend/conftest.py` УЖЕ есть autouse-фикстура `reset_company_context`
(задача 2, расширена в задаче 3). Она делает две вещи: сбрасывает процессный
`ContextVar` до и после каждого теста и возвращает `search_path` в `public`
после теста. **Не дублируйте её** и не оборачивайте фикстуры ниже в
`try/finally` ради сброса контекста или пути — это её работа.

Две детали её устройства, которые легко сломать при правке:
`SET search_path` выполняется только если у теста есть маркер `django_db`
(проверка `connection.connection is not None` тут НЕ работает: атрибут
проходит, а следующий `cursor()` дёргает заблокированный pytest-django
`ensure_connection`), и только если `not connection.needs_rollback` — иначе
`SET` упадёт на транзакции, оставленной в состоянии aborted тестом
`test_slug_is_unique`.
