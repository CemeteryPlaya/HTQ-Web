...
добавленная в задаче 2: она сбрасывает процессный `ContextVar` до и после
каждого теста. **Не дублируйте её** и не оборачивайте фикстуры ниже в
`try/finally` для сброса контекста — это её работа.
