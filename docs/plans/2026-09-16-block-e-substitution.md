...
6. **Строка «Системный администратор» не сеется.** Документ называет его замещающим «Внутригрупповой ИТ-подрядчик» — это не должность и не пользователь платформы, смоделировать его как `substitute_position` нельзя. Сид печатает предупреждение, что строка документа осталась незакрытой; это честнее, чем выдумать должность-заглушку.
7. **Названия в HR-FRM-006 и в оргструктуре расходятся** — это открытый вопрос руководству (roadmap §8.2), и блок E его не закрывает. Для демо-стенда заведено ЯВНОЕ соответствие (задача 6), каждая строка подписана оригиналом из документа; когда руководство ответит, правится одна таблица.
...
```python
    substitutions=(
        ...
        SubstitutionRow("Главный бухгалтер", "Кадровый бухгалтер",
...
def test_substitution_matrix_matches_the_document():
    ...
    assert actual == {
        ...
        ("Главный бухгалтер", "primary", "Кадровый бухгалтер"),
        ("Главный бухгалтер", "reserve", "Экономист-аналитик"),
    }
    assert not any(r.position == "Системный администратор" for r in HOLDING.substitutions)
...
```python
    ...
    def _warn_unmapped_substitutions(self, structure, positions) -> None:
        ...
        if structure.kind != "holding":
            return
        if "Системный администратор" not in positions:
            return
        self.stdout.write(self.style.WARNING(
            "  Строка HR-FRM-006 «Системный администратор → Внутригрупповой "
...
def test_holding_seed_lays_out_the_substitution_matrix(company_schema):
    ...
    with use_company(company_schema["slug"]):
        ...
        assert not Substitution.objects.filter(
            position__title="Системный администратор").exists()
