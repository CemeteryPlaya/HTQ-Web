...
| 10в | `hr.business_trip` | Командировка | `employee_id`, `department_id`, `destination`, `country`, `days`, `estimated_cost`, `date_from`, `date_to` |
  ...
  ⚠️ В компании БЕЗ активных подразделений объявление полей шести предметов
  (у них `department_id` — `choice`) движок отвергает целиком: `choice` без
  вариантов. Редактор маршрутов проглотит это и покажет предмет без условий.
  Самолечится данными — без подразделения не завести ни должность, ни
  сотрудника; но при настройке маршрутов на пустой компании выглядит как
  сломанный редактор (`test_without_departments_the_route_editor_sees_no_conditions`).
...
4. **G** — HR-субъекты (моя половина) после их `ApproverKind`. Семь
   expand-миграций `hr/0027`–`0034` через `migrate_companies`: `0027` —
   ...
   без переписывания таблицы). Данные не переносятся, откат — снос таблиц;
