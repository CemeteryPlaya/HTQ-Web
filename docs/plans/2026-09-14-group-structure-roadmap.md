...
6. **H** (выполнено) — сводки: новое представление `holding.hr_staffingposition`
   ...
   **I** (выполнено в коде) — снятие параллельного RBAC. ⚠️ **Гейты уже в
   ...
   попадёт под трафик:
   0. Чеклист выкатки поддоменов —
      [`docs/deploy/subdomains-runbook.md`](../deploy/subdomains-runbook.md)
      (DNS, origin-сертификат, `SFU_ALLOWED_ORIGINS`, порядок команд,
      проверочные запросы). Поддомены и гейты выкатываются ОДНИМ окном:
      по отдельности они не имеют смысла — без поддоменов гейты блока I
      закрывают всё (на голом домене нет контекста компании → `none`
      всем, кроме суперпользователя), без гейтов поддомены ничего не
      защищают.
