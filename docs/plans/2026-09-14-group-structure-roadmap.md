...
- `seed_group_demo [--skip-tasks]` — единственный локальный способ увидеть группу: заводит холдинг, три ДО, сеет структуры, учётки, членства обслуживающих должностей (блок C), задачи HTQ.
