...
3. `manage.py company_bankrupt --company X --successor Y` — участники X
   ...
   Крупную компанию закрывать командой, а не кнопкой: кнопка выдаёт членства
   синхронно в запросе, а у веб-процесса таймаут 60 с.
4. `manage.py tenancy_status` — слепок после; расхождение — стоп-сигнал.
   Статус компании кэширован на 5 с, поэтому проверять результат (архив X,
   вход в Y) не раньше чем через 5 с после шага 3.
   Если после сбоя X осталась **действующей** с выставленным преемником —
   повторить `company_bankrupt` с тем же преемником (довыдаст членства и
   заархивирует) либо снять преемника у X в django-admin.
...
экраном участников Y руками. Флаг «по умолчанию» тоже не возвращается: у
перенесённых людей две компании «по умолчанию» (X и Y) — проверить и
поправить в django-admin (`CompanyMembershipAdmin`): экран участников флаг
только показывает.
