...
- **Справочники бюджета** — `Country`, `Program` (название + статья расходов в одной строке), `Administrator` (держатель бюджетных строк — **проект + страна**, без ФИО и **без денег на самой записи**; подпись «проект страна» собирает `Administrator.display_name`), `Budget` (выделенная сумма на связку администратор × программа × год, уникальную).
...
`apps.signoff` (URL-префикс `api/signoff/v1/`) — движок согласования, который ничего не знает о предметных аппках. Frontend'а пока нет.
...
**Как подключить новый тип** — три шага, ни одного в самом signoff: модель наследует `signoff.Approvable` и объявляет `SIGNOFF_SUBJECT_TYPE`; миграция добавляет `approval_state`; `AppConfig.ready()` зовёт свой `approval_hooks.register()`. Образец — `apps/contracts/approval_hooks.py`, минимальный — `apps/signoff/tests/testapp/`.
