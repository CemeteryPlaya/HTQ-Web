...
level=…)``: правка контента — ``write``, приглашения — ``read`` (конфиг
конференции — без гейта, ``open`` в реестре самообслуживания). Им нужны заголовок ``X-HTQ-Company``, claim ``company`` в токене,
