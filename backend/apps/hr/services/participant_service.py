...
_PRESCRIBED = {
    ...
    # ``permissions`` больше НЕ проставляется (задача 9 блока I: колонка
    # мертва для авторизации) — держатель ОСУ получает права в компании
    # обычным путём, членством и ролями, как и любой другой (см. докстринг
    # модуля, «руководящая должность с внешней иерархией»).
    "description": (
