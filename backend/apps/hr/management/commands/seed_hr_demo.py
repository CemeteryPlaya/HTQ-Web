...
читаемым. Кадровые данные — через ``update_or_create`` по естественному
ключу, поэтому второй запуск ничего не дублирует, а правит на месте. Роли
должностей — исключение: они выдаются через ``get_or_create``
(``access.interface.ensure_position_role``) — уже стоящая связь, в том числе
с другой областью или выставленная руками, не переписывается.
