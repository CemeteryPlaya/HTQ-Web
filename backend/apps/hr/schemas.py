...
class HRUserCreateRequest(BaseModel):
    """``POST /employees/users/`` body — port of
    ``services/hr/app/api/v1/employees.py::HRUserCreateRequest``. The HR side
    only knows ФИО + email; username/password are derived/generated by
    ``apps.users.interface.create_user``, not asked for here.
