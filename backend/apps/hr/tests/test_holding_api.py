...
Обычный модульный гейт (``module="hr", level="read"``) знает только уровень
доступа ВЫЗЫВАЮЩЕГО и ничего не знает про ВИД его компании, поэтому вьюха
обязана сверить это сама (``views._deny_unless_holding``) — тесты ниже бьют
именно по этому шву.

«HR-доступ» здесь — ``is_staff=True`` БЕЗ ``is_superuser``. ДО задачи 9 блока
I ``token.is_elevated`` в снятом резолвере ``hr_access.resolve_hr_access``
давал такому токену полный HR-доступ без карточки сотрудника; сегодня это
уже не так (см. ``hr_token()`` ниже) — но токен всё равно нарочно РАЗНЫЙ от
платформенного администратора (``is_superuser=True``): только последний
обязан обходить проверку вида компании — обычный кадровик, даже с полным
HR-доступом, ограничен своим поддоменом. Employee-карточками не пользуемся:
они добавили бы отделы и штат компаниям и испортили бы ручной подсчёт totals
ниже.

Блок I, задача 6 поставила ``holding_headcount`` под ``module="hr",
level="read"`` ПОВЕРХ ``hr_access.require_hr_access`` (без замены — обе
двери были открыты разом, «И-И»); задача 9 сняла вторую дверь целиком —
сегодня решает только модульный гейт. Токены здесь — сырой ``pyjwt.encode``
(не ``issue_token_pair``, см. ``token()``/``hr_token()``/``superuser_token()``
ниже), а не настоящие ``User`` строки, поэтому "полный HR-доступ"
``hr_token()`` сам по себе гейт НЕ покрывает —
...
единой привилегии ни там, ни там) получает 403 от ГЕЙТА — единственная
проверка, что осталась, см. ``test_no_hr_access_is_forbidden``.
...
def hr_token(**over) -> str:
    """``is_elevated`` (is_staff/is_admin), НЕ superuser. До задачи 9 блока I
    это давало полный HR-доступ безусловно (снятый резолвер коротил
    ``token.is_elevated`` в wildcard); сегодня сам по себе гейт ``module="hr"``
    не проходит (единственный бесплатный обход там — ``is_superuser``) —
    ``_grant_holding_role`` ниже выдаёт этому ``user_id`` узел ``hr`` явно.
    В любом случае без права обойти проверку вида компании."""
...
def test_no_hr_access_is_forbidden(client, two_companies):
    """Обычный токен без единой роли ``apps.access`` — 403 ещё до проверки
    вида компании, сводка сервиса вообще не вызывается. Detail — общий
    "Forbidden" от гейта ``module="hr"`` (задача 9 сняла ``hr_access.
    require_hr_access``, который раньше стоял ПОВЕРХ этого же гейта и давал
    detail "HR access required" — сегодня та вторая дверь снята, отказывает
    только гейт).

    ``user_id`` здесь СВОЙ (не 7 по умолчанию): ``two_companies`` выдаёт узел
    ``hr`` пользователю 7 ради ``hr_token()``, и «токен без единой роли» с
    тем же id роль бы унаследовал. До задачи 9 тест держался на старой
    проверке (не elevated, без карточки → 403), которая роли не смотрела,
    — теперь смотрит только гейт, и вызывающий обязан быть без роли ПО ФАКТУ.
    """
    resp = client.get(BASE, **headers(HOLDING_SLUG, token(user_id=8, sub="8", company=HOLDING_SLUG)))
    assert resp.status_code == 403
