...
хуже двух. Общий у них только ``--company``/``--dry-run``, и это не повод
их сращивать.

**Только для уже существующих членств.** С финальной волны блока I
(рулинг M) новый участник получает роль в момент создания членства —
``apps.companies.services.membership_service.grant_membership`` зовёт
``apps.access.interface.ensure_basic_role``. Эта команда закрывает тех, кто
стал участником ДО этого (день выкатки), и остаётся идемпотентной страховкой.
