...
#: гейт навешан на все 128 её ручек (``SELF_SERVICE["tasks"]`` остаётся
#: пустым: у домена задач нет самообслуживания, см. комментарий там же).
...
SELF_SERVICE: dict[str, dict[str, str]] = {
    ...
    "hr": {
        ...
        # ``open``). ``_identity_access`` (``apps/hr/views.py``) намеренно
        # НЕ зовёт ``require_hr_access``: право РЕШАТЬ заявку принадлежит
        # подтверждающему (руководителю отдела), который в общем случае НЕ
        # кадровик и не несёт ни одного узла ``hr.*`` — гейт module="hr"
        # отсёк бы его от собственной задачи (см. докстринг
