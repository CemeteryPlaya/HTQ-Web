"""Гейт модуля ``media`` на НАСТОЯЩИХ ролях (блок L, задача 4).

``employee-basic`` (узлы ``access/0004`` + ``0011``) агрегируется в ``write``
модуля ``media`` — загрузка и подпись ему открыты, общий список файлов
(бывший ``admin=True``, теперь ``level="admin"``) — нет. ``is_staff`` без
роли на бывшую админскую ручку больше не пускает (решение заказчика 2);
держатель ``services-admin`` (``access/0012``) — пускает.
"""
