...
class EmailAccount(models.Model):
    ...
    * ``personal`` + ``imap_settings`` — любой другой сервер по IMAP/SMTP,
    ...
    # Подпись сотрудника — текст, которым подставляется конец каждого письма
    # с этого адреса. Хранится ПРИ АККАУНТЕ, а не при пользователе: у одного
    # человека может быть и корпоративный ящик, и личный, и подписывать
    # клиентское письмо тем же, чем личное, он не захочет.
    #
    # Пустая строка, а не NULL: «подписи нет» и «подпись стёрли» — одно и то
    # же состояние, и различать их нечем и незачем.
    signature = models.TextField(blank=True, default="", db_default="")
