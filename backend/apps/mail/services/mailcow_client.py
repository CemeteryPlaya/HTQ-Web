...
class MailcowClient:
    ...
    def probe(self) -> tuple[int, Any]:
        """Сырой ответ на самый дешёвый запрос — для диагностики, не для работы.

        Отдаёт ``(HTTP-код, разобранное тело либо текст)`` и намеренно НЕ
        поднимает ``MailcowError``: код здесь и есть диагноз. 401/403 значит
        «ключ не принят или адрес не в списке разрешённых» — самая частая
        причина «ключ есть, а не работает», — и схлопнуть её в общее «Mailcow
        не ответил» значило бы выбросить единственную деталь, по которой это
        чинится.

        Тело возвращается как есть ещё и потому, что Mailcow **отвечает 200 на
        неудачную аутентификацию** (``{"type": "error", ...}``): по одному
        коду отличить отказ от успеха нельзя, смотреть надо на оба.

        ``/get/domain/all`` выбран как проверочный: он не зависит от того,
        какой домен настроен, и заодно отвечает на следующий вопрос —
        виден ли ключу НАШ домен.
        """
        resp = self._request(
            "GET", f"{self.base_url}/get/domain/all", headers=self._headers(),
        )
        try:
            return resp.status_code, resp.json()
        except ValueError:
            # Не JSON — почти наверняка HTML страницы входа: в адресе забыли
            # /api/v1 и запрос ушёл в панель.
