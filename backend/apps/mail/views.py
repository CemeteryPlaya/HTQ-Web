...
def corporate_connect_info(request):
    ...
    return {
        ...
        # Ящика у сотрудника нет, а рабочий адрес — корпоративный: почти
        # наверняка ящик на сервере есть, просто платформа о нём не знает
        # (учётку завели до автоподключения, или оно не сработало).
        #
        # Утверждать «ящик есть» тут нельзя и не нужно: на голом IMAP нет
        # команды «есть ли такой ящик» — проверить можно ТОЛЬКО логином, а
        # пароля у платформы нет (см. provisioning/imap.py::exists_remote).
        # Поэтому порядок обратный тому, каким его хочется видеть: сначала
        # спрашиваем пароль, а успешный вход и есть доказательство, что
        # ящик существует и принадлежит спрашивающему.
        #
        # ``provisioner == "none"`` исключён: почтового сервера нет вовсе,
        # подключаться некуда, и просьба ввести пароль была бы издевательством.
        "suggest_connect": bool(
            own_address and mailbox is None and info["provisioner"] != "none"
        ),
    }
