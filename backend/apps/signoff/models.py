...
class Approvable(models.Model):
    ...
    def assert_editable(self) -> None:
        """Запретить правку объекта, пока по нему идёт согласование.
        ...
        «pending — значит заморожено» описывает семантику колонки, которую
        объявляет эта примесь. Оставь его в contracts — и следующая
        согласуемая аппка перепишет его своими словами, с собственным
        текстом ошибки и собственным набором забытых операций.

        Молчит в двух случаях:

        * объект не на согласовании — сюда же попадает ``rejected``:
          отклонённый объект дорабатывают и шлют снова, запрещать правку
          ровно тогда, когда она и требуется, было бы бессмысленно;
        * ``signoff`` выключён. По той же логике, что записана в
          ``interface.has_active_route``: выключенный модуль согласования
          перестаёт ТРЕБОВАТЬ согласования, а не запирает подключившие его
          аппки. Здесь у этого есть и практическая причина — отзыв
          процесса стоит за ``require_service("signoff")``, так что без
          этой оговорки выключение модуля навсегда заморозило бы всё, что
          застигнуто в ``pending``, не оставив ни одного способа
          разблокировать.
        ...
        raise SubjectLocked(
            f"{self._meta.verbose_name} находится на согласовании и не "
            f"редактируется — отзовите согласование или дождитесь решения"
        )
