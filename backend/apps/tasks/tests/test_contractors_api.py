"""Субподрядчики — ``/api/tasks/v1/contractors/*`` и атрибуция работ.

Этап «только справочник»: организации, их представители с уровнями и
привлечения на объекты заводятся и ведутся, но входа в систему у подрядчика
...
* техника со статусом «подрядчика» обязана называть подрядчика — правило
...
# ─────────────────────────────────────────────────────────────────────────
# Входа у подрядчиков нет — и не должно появиться незаметно
# ─────────────────────────────────────────────────────────────────────────
...
def test_contractor_level_does_not_affect_task_visibility_yet():
    ...
    # Обычный сотрудник (не участник) задачу подрядчика не видит — ровно
...
def test_engagement_on_a_site_alone_is_valid():
    """Подрядчик может работать на объекте вне конкретного проекта."""
    contractor = _contractor()
...
def test_existing_equipment_reads_as_own():
    """Парк, который вёлся до подрядчиков, честно становится собственным —
    а не «неизвестно»."""
