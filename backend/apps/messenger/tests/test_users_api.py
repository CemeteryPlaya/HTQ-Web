...
``services/messenger/app/api/v1/users.py`` (4 регистрации: me×2 + search×2,
workers/admin под-задача, PLAN.md §6.5, последняя под-задача messenger).

``POST /users/ingest`` снесён — P1.2 audit-спека (2026-07-22): целевой
``chat_user_replicas``/``ChatUserReplica`` в Django нет, приёмнику некуда
писать.

Р2 (me — см. apps/messenger/models.py докстринг файла): исходник читал/писал
``chat_user_replicas``, которая здесь не портируется:
  ...
  * ``GET  /users/search`` — БОЛЬШЕ НЕ деградация: apps.users.interface
    получила list_users_brief (задача Р3 без S2S), search/me вьюха отдаёт
    реальные результаты по apps.users.User (username/first_name/last_name/
    email), исключая текущего пользователя и неактивных.
"""
