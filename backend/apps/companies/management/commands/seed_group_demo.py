...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # С задачи 11 блока I «Единая модель прав» ``seed_hr_demo`` выдаёт
        # роли должностям сам — по ``Post.hr_level`` структуры (ruling H:
        # ровно то, что администратор сделал бы после переноса), поэтому на
        # свежем стенде держатели ЕСТЬ и членства им раздаются здесь же.
        # Ноль возможен, если роли сняли руками или у держателей нет
        # учёток, — и тогда это печатается явно: молчание читалось бы как
        # «наследование сломано».
        #
        # Гейт сообщения — число ДЕРЖАТЕЛЕЙ (serving_holders), а не число
        # НОВЫХ членств за этот прогон: на повторном прогоне новых членств
        # снова ноль — но держатели есть, и повторять «роли не назначены»
        # здесь было бы ложью, отправляющей оператора переделывать уже
        # сделанное.
        ...
        if total_holders == 0:
            self.stdout.write(
                ...
                "должностям холдинга не назначены роли (демо-сид выдаёт их по "
                "hr_level структуры — значит, их сняли или у держателей нет "
                "учёток), а без роли признак «обслуживает дочерние компании» "
                "ничего не выдаёт. Назначьте роль должности на экране "
                "должностей — членства доберёт "
