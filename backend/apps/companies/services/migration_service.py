...
Почему public всё-таки в пути, а не выброшен совсем. Четыре тенантные
миграции зависят от django_celery_beat — аппки НЕтенантной, живущей в
public: hr.0019/hr.0020 и tasks.0003/tasks.0019 заводят и переключают на
диспетчеров периодические задачи. Расписание у платформы одно на всех, beat
читает его из public, поэтому вторая копия этих таблиц в схеме компании была
бы мёртвым грузом, а без public в пути миграции просто падают на
несуществующей таблице. С public вторым в пути их SQL разрешается туда, где
эти таблицы уже есть и где им место.
...
пропущенных шагов не доигрывает), поэтому без этих отметок hr.0019/hr.0020
получили бы состояние без моделей django_celery_beat и упали бы на
...
# hr.0019) — лист графа hr, а лист, помеченный применённым заранее, уводит
