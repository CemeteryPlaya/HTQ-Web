...
def _lock(cur) -> None:
    ...
    Ключ общий с migration_service, поэтому пересборка не пересекается с
    прогоном миграций по схеме. Но лок НЕ удерживается на весь прогон
    команды migrate_companies: она берёт и отпускает его трижды — на сносе,
    на каждой migrate_company и на финальной сборке. Значит параллельная
    пересборка (заведение новой компании в соседнем процессе) всё ещё может
    вклиниться между сносом и миграциями и собрать вьюхи обратно, и тогда
    contract-миграция упадёт на них. Отказ при этом громкий и лечится
    повтором прогона; удержание лока на всю команду потребовало бы
    сессионного уровня и своей обработки сбоя разблокировки.
