...
def drop_schema(slug: str) -> None:
    ...
    Штатным закрытием компании НЕ является: закрытие — это архив, при
    банкротстве — с переносом членств преемнику (подпроект 4), данные при нём
    сохраняются. Эта функция нужна для отката неудавшегося создания и для
    уборки в тестах.
    """
    with connection.cursor() as cur:
        cur.execute(
