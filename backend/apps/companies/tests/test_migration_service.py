...
def test_plan_refuses_backwards_shared_effect_migration(alpha):
    ...
    ``hr.0019`` и ``hr.0020`` (лист графа hr с followup-a, п.1 — перевод
    расписания на диспетчера) — обе в ``SHARED_EFFECT_MIGRATIONS``: после
    полного прогона они отмечены применёнными (без выполнения). Откат до
    предыдущей миграции (target — 0018) даёт план из ДВУХ обратных шагов
    (0020, затем 0019), и оба попадают не в ``steps``, а в ``shared`` —
