...
``0020`` сделал ключом программы один код — реестр договоров это позволял.
...
обязана — отсюда и то, что ``0020`` не правится, а перекрывается этой:
``0020`` уже применена на проде.

Заодно возвращается правило, которое ``0020`` снял вместе со старым ключом:
...
руками, ложатся в базу дважды. ⚠️ Если за время между ``0020`` и этой
...
class Migration(migrations.Migration):
    ...
    dependencies = [("contracts", "0020_agreement_registry_fields")]
