...
Ported verbatim from ``services/requests/app/services/condition_eval.py`` —
it has no framework dependency, and it decides which branch a real approval
takes, so it is copied rather than "improved". Its original rationale still
holds: a hand-rolled subset instead of the unmaintained PyPI ``json-logic``,
which is broken under modern Python.
...
Operators covered:
  ...
  Arithmetic:  +  -  *  /
  Membership:  in
  Variable:    var  (dot-path, e.g. {"var": "items.0.price"})
  Literals:    any non-dict, or a dict without a known op key, passes through.

Structural validation of an expression (that ``var`` refs point at known
field keys) belongs to template validation; evaluation here is deliberately
permissive — a missing variable resolves to ``None`` and most comparisons
against ``None`` yield ``False`` rather than raising, so a half-filled form
cannot crash the engine mid-route.
"""
...
def evaluate_raw(expr: Any, values: dict) -> Any:
    """Evaluate ``expr`` against ``values``, returning the raw result."""
    if not isinstance(expr, dict) or not expr:
        return expr
    if len(expr) != 1:
        # JsonLogic spec: an op-dict has exactly one key; more than one is
...
def evaluate(expr: Any, values: dict) -> bool:
    """Boolean value of ``expr`` evaluated against ``values``."""
