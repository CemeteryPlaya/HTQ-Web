...
Ported verbatim from ``services/requests/app/services/workflow_schema.py``
(no framework dependency). This is the gate that stops a malformed route from
ever being published: a graph that passes here is guaranteed to have one
start, at least one end, no dangling edges and — importantly — no cycle, so
the runtime's traversal cannot loop forever on a legitimate template.
"""
...
def extract_var_refs(expr: Any) -> set[str]:
    """Every ``{"var": "<field>"}`` reference inside a JsonLogic expression."""
    refs: set[str] = set()
