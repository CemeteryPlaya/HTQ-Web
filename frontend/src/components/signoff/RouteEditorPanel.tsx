/**
 * Конструктор маршрута согласования — панель, а не страница.
 *
 * Тот же редактор живёт в двух местах: на странице `/signoff/routes/:id` и
 * внутри конструктора шаблона «Запросов» (шаг «Маршрут»), где маршрут задан
 * областью `template:<id>`. Поэтому здесь нет ни рамки раздела, ни ссылки
 * «ко всем маршрутам» — только сам редактор, принимающий id маршрута.
 *
...
export function RouteEditorPanel({ routeId }: { routeId: number }) {
  ...
  /** Поля для условий объявляет предметная аппка — и объявляет ПО ОБЛАСТИ
   *  маршрута (у каждого шаблона «Запросов» своя форма), поэтому берём их из
   *  карточки маршрута; `subject.fields` — запасной вариант для типов без
   *  областей. То же и с ключами «назначает объект». */
  const fields = route?.fields ?? subject?.fields ?? [];
  const approverFields = route?.approver_fields ?? subject?.approver_fields ?? [];
  const requirementFields = route?.requirement_fields ?? subject?.requirement_fields ?? [];
  ...
  const submitDraft = () => {
    ...
    if (draft.approverKind === 'users' && draft.userIds.length === 0) {
      setDraftError('Выберите хотя бы одного согласующего.');
      return;
    }
    if (draft.approverKind === 'subject' && !draft.approverKey) {
      setDraftError('Выберите, кого объект назначает согласующим.');
      return;
    }
                      ...
                      <div key={stage.id} className="rounded-lg border p-3">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            {stage.requirement_key && (
                              <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                                <ClipboardCheck className="h-3.5 w-3.5 shrink-0" />
                                {stage.requirement_label
                                  ?? `требует от объекта: ${stage.requirement_key}`}
                              </p>
                            )}
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          ...
                          ) : stage.approver_kind === 'subject' ? (
                            <Badge variant="outline" className="text-muted-foreground">
                              <Split className="mr-1 h-3 w-3" />
                              {stage.approver_label
                                || `объект: ${stage.approver_key || '—'}`}
                            </Badge>
                          ) : stage.approver_kind === 'users' ? (
                            (stage.users ?? []).length === 0 ? (
                              <span className="text-sm text-destructive">
                                согласующих нет — этап не исполнится
                              </span>
                            ) : (
                              (stage.users ?? []).map((person) => (
                                <Badge
                                  key={person.user_id}
                                  variant="secondary"
                                  className={person.is_active ? '' : 'opacity-60'}
                                >
                                  {person.full_name || `ID ${person.user_id}`}
                                  {!person.is_active && ' (неактивен)'}
                                </Badge>
                              ))
                            )
              ...
              {draft && (
                <div className="space-y-4">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="users">
                          {APPROVER_KIND_LABELS.users}
                        </SelectItem>
                        {approverFields.length > 0 && (
                          <SelectItem value="subject">
                            {APPROVER_KIND_LABELS.subject}
                          </SelectItem>
                        )}
                      </SelectContent>
                    </Select>
                  ...
                  {/* Требование к ОБЪЕКТУ — третье рядом с документом и
                      ...
                      а что должно быть сделано на самом объекте (заполнен
                      поставщик). Список ключей — от предметной аппки; тип,
                      который ничего такого не умеет, поля не показывает. */}
                  {requirementFields.length > 0 && (
                    <div className="space-y-1.5">
                      <Label htmlFor="stage-requirement" className="text-sm">
                        Этап требует от объекта
                      </Label>
                      <Select
                        value={draft.requirementKey || '__none__'}
                        onValueChange={(value) =>
                          setDraft({ ...draft, requirementKey: value === '__none__' ? '' : value })
                        }
                      >
                        <SelectTrigger id="stage-requirement">
                          <SelectValue placeholder="Ничего" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__none__">Ничего</SelectItem>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Согласовать этап будет можно только когда это сделано —
                        например, заполнено поле, которое вносит сам согласующий
                        на своём шаге. На отказ и доработку не влияет.
                      </p>
                    </div>
                  )}
                  ...
                  {draft.approverKind === 'users' && (
                    <div className="space-y-1.5">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        Названные поимённо: подойдёт там, где согласующий — это
                        конкретный человек, а не должность. Уволенного
                        сотрудника маршрут не примет.
                      </p>
                    </div>
                  )}

                  {draft.approverKind === 'subject' && (
                    <div className="space-y-1.5">
                      <Label>Кого назначает объект</Label>
                      <Select
                        value={draft.approverKey}
                        onValueChange={(value) => setDraft({ ...draft, approverKey: value })}
                      >
                        <SelectTrigger>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Согласующие берутся из самого объекта при запуске —
                        например, из поля формы или списка администраторов
                        проекта. Если объект не назовёт никого, отправка
                        откажет с объяснением.
                      </p>
                    </div>
                  )}
