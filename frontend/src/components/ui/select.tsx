...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 min-h-[44px] sm:min-h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className,
    )}
    {...props}
  >
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        ...
        // Без этого вьюпорт растёт по содержимому, перерастает max-h-96 и
        // молча обрезается `overflow-hidden` — список становится непрокручиваемым.
    ...
    >
      ...
      <SelectPrimitive.Viewport
        className={cn(
          "p-1",
          // Высоту вьюпорта НЕ фиксируем: `h-[--radix-select-trigger-height]`
          // прижимал список к высоте самого поля (~40px), и из десяти отделов
          // был виден один.
