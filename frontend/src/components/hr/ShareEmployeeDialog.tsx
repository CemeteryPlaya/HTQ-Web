  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && close()}>
      <DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] overflow-y-auto sm:max-h-[calc(100dvh-4rem)]">
        <DialogHeader>
          <DialogTitle>
            Поделиться карточкой{employee?.full_name ? ` · ${employee.full_name}` : ''}
          </DialogTitle>
        </DialogHeader>
          ...
          {mode === 'internal' ? (
            <div className="grid gap-3">
              <p className="text-muted-foreground">
                Эту ссылку могут открыть только залогиненные сотрудники с HR-доступом.
              </p>
            ...
            <>
              <label className="grid gap-1.5">
                ...
                <span className="text-xs text-muted-foreground">
                  Покажется поверх карточки при открытии — для отслеживания утечек.
                </span>
              </label>
              <label className="grid gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                В публичной версии скрыты email и телефоны, а ссылки на других
                сотрудников неактивны.
              </p>
              <div className="flex flex-col-reverse gap-2 pt-2 sm:flex-row sm:justify-end">
            ...
            <div className="grid gap-3">
              <p className="text-muted-foreground">
                Скопируйте ссылку сейчас — она показывается только один раз. После
                закрытия окна восстановить её будет нельзя.
              </p>
              <div className="rounded-md border bg-muted/40 p-3 font-mono text-xs break-all">
                {publicUrl}
              </div>
