 ...
 *
 * 1. Захват шёл на всей странице конференции, включая экран выбора комнаты,
 *    где предпросмотра не видно вовсе. Теперь решение принимает страница и
 *    передаёт его сюда через `active`: хук сам никогда не догадывается, что
 *    устройства «наверное, нужны». Кто и когда ставит `active` — в
 *    ConferencePage.tsx (сейчас это комната ожидания).
...
export interface PreviewMediaOptions {
  /** Экран, на котором предпросмотр виден. Без этого не захватывается ничего. */
  active: boolean;
  cam: boolean;
  mic: boolean;
