    ...
    <header className="shrink-0 border-b bg-card">
      <div className="mx-auto flex h-14 max-w-5xl items-center gap-2 px-3 sm:gap-4 sm:px-6">
        ...
        <div className="min-w-0 flex-1 truncate text-xs text-muted-foreground">
          <span className="hidden md:inline">Карточка сотрудника</span>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          setErrorMsg('Произошла ошибка. Обратитесь к отправителю ссылки.');
          setState('error');
          return;
        }
    ...
    return (
      <div className="flex min-h-[100dvh] flex-col bg-background">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-sm text-muted-foreground">Загрузка карточки...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-[100dvh] flex-col bg-background">
      ...
      <footer className="shrink-0 border-t bg-card px-3 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] text-[11px] text-muted-foreground sm:px-6 sm:text-xs">
        <div className="mx-auto flex max-w-5xl flex-col gap-1 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
          <span>Просмотр ограничен. Данные доступны только по этой ссылке.</span>
          {data?.generated_at && (
            <span>
              Сгенерировано {new Date(data.generated_at).toLocaleString('ru')}
            </span>
          )}
        </div>
      </footer>
