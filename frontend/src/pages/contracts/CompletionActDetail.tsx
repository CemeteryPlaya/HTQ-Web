...
// Формулировки те же, что в CompletionActDetailView: один акт не должен
// называться по-разному на карточке и во врезке согласования.
...
export default function CompletionActDetail() {
  ...
  const { activeProfile } = useActiveProfile(); const { hasPerm } = useHRLevel();
  const canRecord = hasAnyRole(activeProfile?.roles ?? [], ['admin', 'superuser', 'staff']) || hasPerm(PERMISSION);
  ...
  return <ContractsShell><BackLink to="/contracts/completion-acts">К актам</BackLink><div className="space-y-6"><div className="flex items-center gap-2"><FileCheck2 className="h-7 w-7 text-muted-foreground" /><h1 className="text-3xl font-bold">Акт выполненных работ</h1></div>
    ...
    <SubmitForApproval subjectType="contracts.completion_act" subjectId={act.id} state={act.approval_state} submit={contractsApi.submitCompletionAct} blockedReason={draftOnlySubmitBlock(act.status, statusLabel[act.status] ?? act.status, 'акт')} invalidate={[["contracts", "completion-acts"]]} /><SubjectProcesses subjectType="contracts.completion_act" subjectId={act.id} />
  </div></ContractsShell>;
}
