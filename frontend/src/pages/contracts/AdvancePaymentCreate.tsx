...
const AdvancePaymentCreate = () => {
  ...
  // «Договор есть, а в списке его нет» — самый частый вопрос по этой форме:
  // сюда попадают только согласованные. Разводим два случая, потому что
  // делать с ними надо разное: оформить договор или дождаться согласования.
  const noAgreements = !isLoading && agreements.length === 0;
  const nothingApproved = !isLoading && agreements.length > 0 && approved.length === 0;
  ...
  // Остаток по договору форма уже знает, поэтому и говорит о превышении сама.
  ...
  const create = useMutation({
    ...
    onError: (err) => reportApiError(err, 'Не удалось создать предоплату'),
  });
  ...
  return <ContractsShell><div className="max-w-2xl">
    ...
    <PrerequisiteNotice
      title="Пока предоплату оформлять не по чему:"
      items={[
        { when: noAgreements, text: 'Договоров пока нет —', to: '/contracts/agreements/new', linkText: 'оформите договор' },
        { when: nothingApproved, text: 'Ни один договор ещё не согласован — предоплата оформляется только по согласованному,', to: '/contracts/agreements', linkText: 'проверьте статусы' },
      ]}
    />
    <form onSubmit={(event) => { event.preventDefault(); if (!agreementId || invalidAmount || exceedsAgreement) { toast.error('Заполните договор и сумму'); return; } create.mutate(); }}>
      <Card><CardHeader><CardTitle>Основание и сумма</CardTitle><CardDescription>После сохранения документ можно отправить на отдельное согласование.</CardDescription></CardHeader><CardContent className="space-y-5">
        ...
        {selected && <div className="rounded-md border bg-muted/40 p-4 text-sm"><div className="flex justify-between gap-4"><span className="text-muted-foreground">Контрагент</span><span>{selected.counterparty_name}</span></div><div className="mt-2 flex justify-between gap-4"><span className="text-muted-foreground">Сумма договора</span><span className="tabular-nums">{selected.contract_type === 'framework' ? 'Открытый договор' : `${formatAmount(selected.amount)} ${selected.currency}`}</span></div><div className="mt-2 flex justify-between gap-4"><span className="text-muted-foreground">Доступно к предоплате</span><span className="font-medium tabular-nums">{formatRemaining(selected.remaining_amount, selected.currency)}</span></div></div>}
