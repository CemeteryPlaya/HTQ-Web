...
export default function GoodsInvoiceCreate() {
  const navigate = useNavigate(); const queryClient = useQueryClient();
  const [administratorId, setAdministratorId] = useState(''); const [agreementId, setAgreementId] = useState('');
  const [amount, setAmount] = useState(''); const [waybill, setWaybill] = useState<File | null>(null);
  ...
  // Накладная составляется по договору, договор — по бюджету. Пустые селекты
  ...
  const create = useMutation({
    mutationFn: () => contractsApi.createGoodsInvoice(Number(administratorId), Number(agreementId), amount.replace(',', '.'), waybill!).then(r => r.data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['contracts'] }); toast.success('Товарная накладная создана'); navigate('/contracts/goods-invoices'); },
    onError: (err) => reportApiError(err, 'Не удалось создать накладную'),
  });
  return <ContractsShell><div className="max-w-2xl"><Link to="/contracts/goods-invoices" className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" />К накладным</Link>
    <div className="mb-6 flex items-center gap-3"><Package className="h-7 w-7 text-muted-foreground" /><h1 className="text-3xl font-bold">Товарная накладная</h1></div>
    <PrerequisiteNotice
      title="Пока накладную составлять не по чему:"
      items={[
        ...
        { when: nothingApproved, text: 'Ни один договор не согласован — накладная составляется только по согласованному действующему договору,', to: '/contracts/agreements', linkText: 'проверьте статусы' },
      ]}
    />
    <form onSubmit={e => { e.preventDefault(); if (!administratorId || !agreementId || !waybill || invalidAmount) { toast.error('Заполните все поля и проверьте сумму'); return; } create.mutate(); }}><Card><CardHeader><CardTitle>Основание и сумма</CardTitle></CardHeader><CardContent className="space-y-5">
      ...
      <div><Label>Накладная</Label><Input type="file" onChange={e => setWaybill(e.target.files?.[0] ?? null)} />{waybill && <p className="mt-1 text-xs text-muted-foreground">{waybill.name}</p>}</div>
    </CardContent></Card><div className="mt-6 flex gap-3"><Button disabled={create.isPending}>{create.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Создать</Button><Button type="button" variant="outline" onClick={() => navigate('/contracts/goods-invoices')}>Отмена</Button></div></form>
  </div></ContractsShell>;
}
