...
export default function AccountableFundsRequestDetail() {
  ...
  // Сколько ещё можно подтвердить — на экране рядом («Осталось подтвердить»),
  // поэтому и превышение форма называет сама, не отправляя запрос.
  const exceedsRemaining = overLimit(
    reportAmount, request?.remaining_accountable_amount);
  ...
  return <ContractsShell><BackLink to="/contracts/accountable-funds-requests">К заявкам</BackLink><div className="space-y-6">
    ...
    {request.budget_line_id == null && <Card><CardHeader><CardTitle>Строка бюджета</CardTitle><CardDescription>Выберите бюджет, с которого должна быть зарезервирована эта заявка. Доступны только строки исходных администратора и программы.</CardDescription></CardHeader><CardContent className="flex flex-wrap items-center gap-3"><Select value={budgetLineId} onValueChange={setBudgetLineId} disabled={!canAssignBudgetLine}><SelectTrigger className="w-full sm:w-96"><SelectValue placeholder="Выберите год и валюту бюджета" /></SelectTrigger><SelectContent>{matchingLines.map((line) => <SelectItem key={line.id} value={String(line.id)}>{line.period_year} — {line.currency}: доступно {formatAmount(line.remaining)}</SelectItem>)}</SelectContent></Select>{canAssignBudgetLine ? <Button disabled={!budgetLineId || assignBudgetLine.isPending} onClick={() => assignBudgetLine.mutate()}>{assignBudgetLine.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Привязать</Button> : <p className="text-sm text-muted-foreground">Привязать строку может инициатор заявки или администратор.</p>}
      <PrerequisiteNotice className="w-full" variant="inline" items={[{ when: canAssignBudgetLine && matchingLines.length === 0, text: 'Подходящих строк нет: у этого администратора и программы нет действующего бюджета —', to: '/contracts/budgets/new', linkText: 'создайте бюджетную строку' }]} />
    </CardContent></Card>}
