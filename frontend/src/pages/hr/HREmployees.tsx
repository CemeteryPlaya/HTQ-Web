...
const HREmployees = () => {
  ...
  // Диплинк `/hr/employees?action=new` — им пользуется плитка «Добавить
  // сотрудника» на панели быстрых действий (HRQuickActionsBar). Параметр
  // снимаем сразу, как и `edit`, чтобы возврат «назад» не открывал форму
  // повторно. Проверка права здесь обязательна: ссылку можно набрать руками.
  useEffect(() => {
    if (searchParams.get('action') !== 'new' || !canCreateEmployee) return;
    startCreate();
    setSearchParams((prev) => {
      const next = new URLSearchParams(prev);
      next.delete('action');
  ...
  }, [searchParams, setSearchParams, canCreateEmployee]);
    ...
    return (
      <HRLayout title={t('hr.pages.employees.title')} subtitle={t('hr.pages.employees.subtitle')}>
        <div className="rounded-2xl border bg-card/70 p-8 text-center text-red-500">
          ...
          <p>{(error instanceof Error ? error.message : '') || t('hr.common.unknownError')}</p>
        </div>
      </HRLayout>
    );
  }
