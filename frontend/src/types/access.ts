...
export interface AccessMe {
   ...
   * `permissions`/`depth` выше (задача 6 блока C, §4.5 расширен аддитивно).
   *
   * Список слагов, а не одна строка: предки не взаимоисключающи (решение
   * заказчика 7) — роли всех обслуживающих предков складываются, и один слаг
   * лгал бы, когда их больше одного. Отсортирован; пусто у суперпользователя
   * (его права ниоткуда не наследуются) и у всех, кому наследование ничего не
   * дало. Существует затем, чтобы права, приехавшие из другой компании, были
   * объяснимы, а не молчаливы — правило «объясняй, не молчи», общее для всей
   * платформы.
   */
