...
Microservice for HTQWeb platform — handles **Approval requests (Lark-style) microservice**.

## Architecture

```
Nginx (Gateway) → /api/requests/* → This Service (FastAPI)
                                              ↓
...
```bash
docker build -t htqweb/requests:latest .
docker run -p 8001:8001 --env-file .env htqweb/requests:latest
```
...
| GET | `/api/requests/` | List items (example) |
| POST | `/api/requests/` | Create item (example) |
...
1. Legacy data was copied to this service's schema (`requests`)
