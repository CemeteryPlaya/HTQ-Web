"""S3 storage helper for cms-service.

Mirrors the abstraction in ``services/media/app/storage.py`` and
``services/messenger/app/services/s3_storage.py``. Per ``services/README.md``
the platform deliberately avoids a shared library — each storage-owning
service ships its own copy so it can evolve independently.

Layout under bucket ``htqweb-cms``::
