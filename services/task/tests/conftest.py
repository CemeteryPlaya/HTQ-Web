...
async def async_engine(pg_url: str):
    engine = create_async_engine(
        ...
        echo=False,
        poolclass=NullPool,
        connect_args={"server_settings": {"search_path": "task,public"}},
    )
    async with engine.begin() as conn:
        await conn.execute(text("CREATE SCHEMA IF NOT EXISTS task"))
        await conn.execute(text("SET search_path TO task, public"))
        for ddl in (
            "DO $$ BEGIN CREATE TYPE versionstatus AS ENUM ('planned','in_progress','released','archived'); EXCEPTION WHEN duplicate_object THEN null; END $$;",
            "DO $$ BEGIN CREATE TYPE status AS ENUM ('open','in_progress','in_review','done','closed'); EXCEPTION WHEN duplicate_object THEN null; END $$;",
            "DO $$ BEGIN CREATE TYPE priority AS ENUM ('critical','high','medium','low','trivial'); EXCEPTION WHEN duplicate_object THEN null; END $$;",
            "DO $$ BEGIN CREATE TYPE tasktype AS ENUM ('task','bug','story','epic','subtask'); EXCEPTION WHEN duplicate_object THEN null; END $$;",
            "DO $$ BEGIN CREATE TYPE linktype AS ENUM ('blocks','is_blocked_by','relates_to','duplicates'); EXCEPTION WHEN duplicate_object THEN null; END $$;",
        ):
...
def make_admin_token(user_id: int = 1, secret: str | None = None) -> str:
...
def make_user_token(user_id: int = 2, secret: str | None = None) -> str:
