"""S3 storage helper for email-service.

Per-service copy following the platform rule (``services/README.md``):
each storage-owning service ships its own ``s3_storage.py`` so it can
evolve independently of media/cms/messenger.

Layout under bucket ``htqweb-mail-attachments``::

    inbound/<account_id>/<message_id>/<filename>
"""
...
def get_storage() -> Storage:
    ...
    return LocalStorage(base_dir=Path(settings.email_local_storage_dir))
