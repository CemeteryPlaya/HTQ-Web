"""HR permission keys + level presets.

Keys are the authoritative unit of HR authorization. ``hr_level`` is a UI /
migration preset only: selecting a level fills a position's ``permissions[]``
with the corresponding preset. Enforcement (``HRAccess.has``) reads keys.
"""
...
CARD_CERTS_VIEW = "hr.card.certs.view"
CARD_CERTS_EDIT = "hr.card.certs.edit"
...
ALL_KEYS: frozenset[str] = frozenset({
    ...
    CARD_CERTS_VIEW, CARD_CERTS_EDIT, CARD_GROUPS_VIEW, CARD_GROUPS_EDIT,
...
def expand_level(level: str | None) -> frozenset[str]:
    """Return the preset key-set for a level; empty set for unknown/None."""
    if not level:
        return frozenset()
